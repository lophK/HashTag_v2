...
router.post('/register_ac', async function  (req, res, next) {
    ...
    if (req.body !=null) {
        ...
        db.query(sql, function (err, data, fields) {
           ...
           //console.log(data);
            if(data.length <=0){
                db.query('INSERT INTO account_user (first_name,last_name,username,password,tel_phone,address,birthday) VALUES (?,?,?,?,?,?,?)', [fname, lname, user, phash, tel, add, bir], function (error, results, fields) {
                    if (results.length >= 1) {
                        res.send('Error');       
                    } else {
